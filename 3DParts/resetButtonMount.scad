...
module SwitchHolder(){
	...
	translate([0,0,0]){
		difference(){
			...
			union(){
				translate([(baseWidth/2)-6, -(baseLength/2)+.5, 0])
					cylinder(d=2,h=20,center=true);		
				mirror([90, 0, 0])
					translate([(baseWidth/2)-6, -(baseLength/2)+.5, 0])
						cylinder(d=2,h=20,center=true);		
			}
			translate([0,3.5,1.8])								// Switch Cut Out
				cube([baseWidth+1,baseLength*3,2],center=true);
		}
		translate([0, -.45 , 1.7]){		// BackBrace
			plate4Point(5.5, 3, 5 ,2);
